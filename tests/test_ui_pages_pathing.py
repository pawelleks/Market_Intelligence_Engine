...
def test_pages_exist_and_home_imports(monkeypatch):
    root = Path(__file__).resolve().parents[1]
    monkeypatch.syspath_prepend(str(root))

    pages = [
        root / "app/pages/01_Market_Regime_Dashboard.py",
        root / "app/pages/02_Regime_Research_Lab.py",
        root / "app/pages/03_Alpha_Signals_Lab.py",
        root / "app/pages/04_Data_Control_Panel.py",
    ]
    for p in pages:
...
def test_home_page_links_use_pages_relative(monkeypatch):
    root = Path(__file__).resolve().parents[1]
