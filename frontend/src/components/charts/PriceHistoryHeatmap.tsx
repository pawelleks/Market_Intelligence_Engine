...
export const PriceHistoryHeatmap: React.FC<PriceHistoryHeatmapProps> = ({ data, ticker }) => {
    const traces = useMemo(() => {
        if (!data?.heatmap?.prob_above || !data?.history) return null;
        ...
        // 7. Candlestick trace (history on the left side, negative x values)
        if (history.length > 0) {
            traceList.push({
                type: 'candlestick',
                x: history.map(h => h.x),
                ...
                increasing: { line: { color: '#d1d5db' }, fillcolor: '#d1d5db' },
                decreasing: { line: { color: '#d1d5db' }, fillcolor: '#d1d5db' },
                name: 'OHLC',
                showlegend: true,
            });
        }
