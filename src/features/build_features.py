...
from src.data_ingest.yfinance_loader import load_registry, save_registry
...
def _compute_features_from_raw(raw_df):
    ...
    raw_df must contain 'date', 'adj_close' and 'ticker'.
    """
    import numpy as np
    import pandas as pd
    ...
    # log return
    df["log_ret_1d"] = np.log(close).diff()
    ...
    # distance from MAs
    if 50 in sma_ws:
        df["dist_from_50dma"] = (close / df["sma_50"]) - 1
    else:
        df["dist_from_50dma"] = np.nan
    if 200 in sma_ws:
        df["dist_from_200dma"] = (close / df["sma_200"]) - 1
    else:
...
def build_features_for_ticker(ticker: str, mode: str = "full", lookback: int = 90, write_csv: bool = False) -> Dict[str, any]:
    ...
    if mode == "full" or not p_feat.exists():
        ...
        if missing:
            LOG.info("Migration: existing features for %s missing %s — will (re)compute from adj_close", ticker, missing)
        ...
        # Ensure continuity for pct_change-based features: recompute ret_1d and log_ret_1d from raw adj_close across the combined df
        try:
            import numpy as np
            # Merge adj_close from raw_df (left join)
            raw_adj = raw_df[["date", "adj_close"]].copy()
            merged = combined.merge(raw_adj, on="date", how="left")
            ...
            merged["ret_1d"] = merged["adj_close"].pct_change()
            merged["log_ret_1d"] = np.log(merged["adj_close"]).diff()
            nan_after = merged["ret_1d"].isna().sum()
            LOG.info("Recomputed ret_1d continuity: nan_before=%s, nan_after=%s", nan_before, nan_after)
            ...
            # Recompute rv_20d for the entire combined (ensures presence and dtype);
            # this preserves warm-up NaNs naturally.
            combined["rv_20d"] = pd.to_numeric(combined["ret_1d"], errors="coerce").rolling(window=20, min_periods=1).std()
            # Also recompute rv_60d if present in schema
