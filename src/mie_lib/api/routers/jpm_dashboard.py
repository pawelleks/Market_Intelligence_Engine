...
# Scaling Factors (Raw -> Display)
# e.g. 0.001 to convert Millions to Billions, or Units to Thousands (K)
SCALING_FACTORS = {
    # Fed Assets (Millions -> Billions)
    'WALCL': 0.001,
    'TREAST': 0.001,
    'WSHOMCB': 0.001,
    
    # Claims (Units -> Thousands)
    'ICSA': 0.001,
    'CCSA': 0.001,
    
    # Retail Sales / Durable Goods (Millions -> Billions)
    'RSAFS': 0.001,
    'NEWORDER': 0.001,
    'RRSFS': 0.001,
    'RSXFS': 0.001,
    'BOPGSTB': 0.001, # Millions -> Billions for Trade Balance
    'EXPGS': 0.001,
    'IMPGS': 0.001,
    
    # Housing (Thousands -> K) - Often already in K, check raw. 
    # HOUST raw is "Thousands of Units". So 1500 means 1.5M. 
    # Display unit is 'K'. If we want 1,500K, factor is 1. If we want 1.5M, factor is 0.001.
    # Current code uses 'K'. Let's assume raw 1500 is fine for '1500 K'.
    
    # SPY (Price -> Price) - No scaling needed, just Unit fix.
}
...
# --- Health Score and Logic Helpers ---
# Removed duplicate calculations in favor of mie_lib.analytics.jpm_dashboard.scoring
...
async def get_overview(response: Response) -> Dict[str, Any]:
    ...
    """
    # Add cache headers (Disable cache to ensure fresh data/structure)
    ...
    for category in CATEGORIES.keys():
        try:
            ...
            # SCALING FIX: Apply scaling factor if exists
            display_value = current_value
            if primary_col in SCALING_FACTORS:
                display_value = current_value * SCALING_FACTORS[primary_col]
            
            # --- Growth Rates Calc ---
            # Need to calc growth early for GDP override
            growth_rates = calculate_growth_rates(clean_df[primary_col], 'quarterly' if category == 'gdp' else 'monthly') # approx freq

            # GDP FIX: If GDP, replace Level with Growth Rate for display
            # Because unit is "% QoQ Ann."
            if category == 'gdp' and growth_rates['last_period_pct'] is not None:
                 display_value = growth_rates['last_period_pct']
                 # Also ensure unit matches
                 # unit = "%" (handled later by standardized UNIT)
            ...
            indicators.append({
                ...
                'sparkline': sparkline,  # Sparkline uses raw values but we could scale them? For now left raw or standard.
                                         # Ideally sparkline should match display unit. 
                                         # Let's simple-scale sparkline if primary uses scaling.
    ...
    for col in series_cols:
        if col != primary_col:
            ...
            secondary_metrics.append({
                ...
                'current_value': float(current_val) * SCALING_FACTORS.get(col, 1.0) if pd.notna(current_val) else None, # SCALED
                'current': float(current_val) * SCALING_FACTORS.get(col, 1.0) if pd.notna(current_val) else None, # key for backward compatibility
                'unit': secondary_unit,
