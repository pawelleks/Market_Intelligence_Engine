# BUSINESS CYCLE - Prediction Analysis Report

**Generated**: 2026-01-11 11:25  
**Analysis Period**: 1981-07-31 00:00:00 to 2025-09-30 00:00:00  
**Data Coverage**: 44.2 years

---

## Executive Summary

**Overall Performance Rank**: #6 out of 9 models  
**Overall Score**: 21.7/100

### Key Metrics at a Glance

| Metric | Value | Assessment |
|--------|-------|------------|
| **Hit Rate** | 16.7% | ❌ Poor |
| **False Positive Rate** | 75.0% | ❌ High |
| **Average Lead Time** | 5.0 months | ⚠️ Acceptable |
| **Avg 12m Return After Signal** | 2.6% | ⚠️ Blow-off Top Pattern |
| **Total Signals Generated** | 4 | - |
| **Recessions Analyzed** | 6 | - |
| **Recessions Caught** | 1 | - |

---

## Recession Prediction Performance

### Summary Statistics

- **Hit Rate**: 16.7% (1 of 6 recessions predicted)
- **Average Lead Time**: 5.0 months
- **Lead Time Range**: 5 to 5 months

### Recession-by-Recession Analysis

| Recession | Start Date | Signal? | Lead Time | Assessment |
|-----------|------------|---------|-----------|------------|
| 1980 | 1980-02 | ❌ No | N/A | Missed |
| 1981-82 | 1981-08 | ❌ No | N/A | Missed |
| 1990-91 | 1990-08 | ❌ No | N/A | Missed |
| 2001 | 2001-04 | ✅ Yes | 5mo | Late warning |
| 2008-09 | 2008-01 | ❌ No | N/A | Missed |
| 2020 | 2020-03 | ❌ No | N/A | Missed |

### False Positive Analysis

**False Positive Rate**: 75.0% (3 of 4 signals)

**Assessment**: ❌ **High false positive rate** - This model triggers frequently without recessions. Best used as an early warning system, not a definitive signal.

---

## Market Performance After Signals

### Average Returns Post-Signal

| Time Horizon | Average Return | Min | Max |
|--------------|----------------|-----|-----|
| **6 Months** | -7.3% | -34.7% | 18.9% |
| **12 Months** | 2.6% | -30.4% | 40.0% |
| **24 Months** | 5.6% | -34.0% | 43.1% |

### Drawdown Analysis

- **Average Max Drawdown**: -25.7%
- **Worst Drawdown**: -48.8%
- **Average Time to Bottom**: 9.6 months

### Blow-off Top Analysis

**Blow-off Tops Detected**: 2 out of 4 signals (50.0%)

✅ **Low Blow-off Top Risk**: This model's signals tend to be followed by actual market declines without significant rallies.

**Implication**: More directly predictive of market downturns.

**Trading Strategy**: Can be used more tactically for market timing and hedging.

---

## Signal Characteristics

**Total Observations**: 531  
**TROUBLE Periods**: 53 (10.0% of time)  
**WARNING Periods**: 136  
**CLEAR Periods**: 342

---

## Key Findings & Recommendations

### Strength: Actionable Lead Time

- ✅ Short lead time means timely signals
- ✅ Less risk of "too early" positioning
- ✅ Better for tactical trading

### Recommended Use Cases

- **Supporting Indicator**: Use as part of a broader ensemble
- **Tactical Trading**: Suitable for near-term positioning

### Limitations

- ⚠️ Misses 83% of recessions - don't rely on this model alone
- ⚠️ High false positive rate (75%) - many signals don't lead to recessions

---

**Report Generated**: 2026-01-11 11:25:27  
**Data Source**: Path B Prediction Analysis Framework  
**For questions or refinements**: See `docs/analysis/`
