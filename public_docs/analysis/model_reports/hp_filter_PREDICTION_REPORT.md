# HP FILTER - Prediction Analysis Report

**Generated**: 2026-02-26 03:02  
**Analysis Period**: 1970-01-01 00:00:00 to 2025-07-01 00:00:00  
**Data Coverage**: 55.5 years

---

## Executive Summary

**Overall Performance Rank**: #1 out of 9 models  
**Overall Score**: 31.7/100

### Key Metrics at a Glance

| Metric | Value | Assessment |
|--------|-------|------------|
| **Hit Rate** | 12.5% | ❌ Poor |
| **False Positive Rate** | 87.5% | ❌ High |
| **Average Lead Time** | 11.0 months | ✅ Optimal |
| **Avg 12m Return After Signal** | 24.3% | ⚠️ Blow-off Top Pattern |
| **Total Signals Generated** | 8 | - |
| **Recessions Analyzed** | 8 | - |
| **Recessions Caught** | 1 | - |

---

## Recession Prediction Performance

### Summary Statistics

- **Hit Rate**: 12.5% (1 of 8 recessions predicted)
- **Average Lead Time**: 11.0 months
- **Lead Time Range**: 11 to 11 months

### Recession-by-Recession Analysis

| Recession | Start Date | Signal? | Lead Time | Assessment |
|-----------|------------|---------|-----------|------------|
| 1970 | 1970-01 | ❌ No | N/A | Missed |
| 1973-75 | 1973-12 | ❌ No | N/A | Missed |
| 1980 | 1980-02 | ❌ No | N/A | Missed |
| 1981-82 | 1981-08 | ❌ No | N/A | Missed |
| 1990-91 | 1990-08 | ❌ No | N/A | Missed |
| 2001 | 2001-04 | ❌ No | N/A | Missed |
| 2008-09 | 2008-01 | ❌ No | N/A | Missed |
| 2020 | 2020-03 | ✅ Yes | 11mo | Good timing |

### False Positive Analysis

**False Positive Rate**: 87.5% (7 of 8 signals)

**Assessment**: ❌ **High false positive rate** - This model triggers frequently without recessions. Best used as an early warning system, not a definitive signal.

---

## Market Performance After Signals

### Average Returns Post-Signal

| Time Horizon | Average Return | Min | Max |
|--------------|----------------|-----|-----|
| **6 Months** | 13.0% | -9.8% | 29.8% |
| **12 Months** | 24.3% | -6.2% | 56.3% |
| **24 Months** | 47.3% | 29.3% | 80.0% |

### Drawdown Analysis

- **Average Max Drawdown**: -9.8%
- **Worst Drawdown**: -22.2%
- **Average Time to Bottom**: 3.2 months

### Blow-off Top Analysis

**Blow-off Tops Detected**: 8 out of 8 signals (100.0%)

⚠️ **High Blow-off Top Rate**: Markets rallied **50.8% on average** in the **22.9 months** after this model signaled TROUBLE before eventually declining.

**Implication**: Recession signals from this model do NOT mean immediate market crash. Expect a rally first, then gradual decline.

**Trading Strategy**: 
- Don't short immediately when signal triggers
- Consider selling rallies into strength
- Use options strategies (sell calls, buy puts with 6-12mo expiry)
- Reduce equity exposure gradually, not all at once

---

## Signal Characteristics

**Total Observations**: 223  
**TROUBLE Periods**: 26 (11.7% of time)  
**WARNING Periods**: 52  
**CLEAR Periods**: 145

---

## Key Findings & Recommendations

### ⭐ Top Performer

This model ranks in the **top 3** of all tested models. 

### Recommended Use Cases

- **Secondary Confirmation**: Use to confirm signals from other models
- **Tactical Trading**: Suitable for near-term positioning

### Limitations

- ⚠️ Misses 88% of recessions - don't rely on this model alone
- ⚠️ High false positive rate (88%) - many signals don't lead to recessions
- ⚠️ Very high blow-off top rate (100%) - signals don't mean immediate market crashes

---

**Report Generated**: 2026-02-26 03:02:36  
**Data Source**: Path B Prediction Analysis Framework  
**For questions or refinements**: See `docs/analysis/`
